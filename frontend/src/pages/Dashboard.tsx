...
export default function DashboardPage() {
  ...
  if (error) return <ErrorState message={error} onRetry={() => (studentId ? load() : nav('/'))} />
  if (!data) return null
    ...
    <div className="page">
      <header className="page-header">
        <div>
          ...
          <p className="muted">分数单元 · {gradeLabel(data.student.grade)} · 学习仪表盘</p>
        </div>
      ...
      {needsOnboarding ? (
        <section className="card hero-cta">
          ...
          <Link to="/diagnose" className="btn primary">开始诊断测评</Link>
        </section>
      ) : (
      ...
      <div className="action-grid">
        <Link to="/diagnose" className="action-card">
          ...
          <ul className="action-list">
            {data.due_reviews.map((d) => (
              <li key={d.skill_id}>
                ...
                <Link to="/practice" className="btn sm">去复习</Link>
              </li>
            ))}
          </ul>
        </section>
          ...
          <div className="weak-list">
            {data.weak_skills.map((w) => (
              <div key={w.skill_id} className="weak-item">
                ...
                <Link to="/practice" className="btn sm">练习</Link>
              </div>
            ))}
          </div>
        </section>
